  ...
  const [rewards, attendanceCount] = await Promise.all([
    prisma.reward.findMany({
      ...
      include: {
        business: true,
        redemptions: {
          where: { userId: user.id, status: { in: ["RESERVED", "REDEEMED"] } },
          select: { id: true }
        }
      }
    }),
    prisma.attendance.count({ where: { userId: user.id, status: "PRESENT" } })
  ]);
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-7 text-slate-300">
                  اطلاعات آن در بخش «مزایای دریافت‌شده» پروفایل تو نگهداری
                  می‌شود.
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-3">
          {rewards.length === 0 ? (
            ...
            rewards.map((reward) => {
              ...
              const eligible = isRewardEligible(reward, {
                attendanceCount,
                ...
                <UserCard key={reward.id} className="flex h-full flex-col">
                  ...
                  <div className="mt-3 grid gap-2 text-xs text-slate-300">
                    ...
                    <span className="flex items-center gap-2 rounded-xl bg-white/[0.05] px-3 py-2">
                      ...
                      />
                      تا{" "}
                      {new Intl.DateTimeFormat("fa-IR").format(reward.expireAt)}
                    </span>
                  </div>
